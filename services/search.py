...
class SearchService:
    ...
    def search_in_index(self, index_name, start=0, hits_per_page=20, data_type=None, query='', ids=[], secondary_region=None, cell_type=None, species=None) -> []:
        try:
            return self.storage.search(index_name, start, hits_per_page, data_type, query, ids, secondary_region, cell_type, species)
        except Exception as ex:
            print(ex)
            raise ex
