...
@routes_api.route('/search/dataset/connection', methods=['POST', 'GET'])
def search_connection(index_name, page=0, hits_per_page=20):
    try:
        data = {}
        if request.method == 'POST':
            data = request.get_json()
        ...
        search_service = Injector(AppModule).get(SearchService)
        query, pre, post = parse_connections_args(data)
        result = search_service.search_connections(start, hits_per_page, query, pre, post)
...
def search(index_name, page=0, hits_per_page=20):
    try:
        ...
        start = page * hits_per_page
        search_service = Injector(AppModule).get(SearchService)
        data_type, query, region, cell_type, species, channels, receptors = parse_query_args(data)
        result = search_service.search_in_index(index_name, start, hits_per_page, data_type, query, [], region, cell_type, species, channels, receptors)
        total_items = result['hits']['total']['value']
        total_page = math.ceil(total_items / hits_per_page)
        response = {
            'total': result['hits']['total']['value'],
